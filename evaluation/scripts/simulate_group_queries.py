...
from apps.cli.runtime_utils import (
    http_post,
    login_and_get_token,
    start_csp_servers,
    stop_processes,
...
def run_client(query: str, username: str, password: str, csp_urls: list[str]) -> tuple[int, str]:
    client_path = os.path.join(PROJ_ROOT, "apps", "cli", "client.py")
    cmd = [
        sys.executable,
        client_path,
        "--query",
...
def main() -> None:
    ...
    ap.add_argument("--aui", type=str, default=os.path.join(PROJ_ROOT, "apps", "cli", "aui.pkl"))
    ap.add_argument("--user-db", type=str, default=os.path.join(PROJ_ROOT, "apps", "cli", "users_db.json"))
